...
// ───────────────────────────────────────────────────────────────────────
// Gedeelde server-logica voor elke dedicated badkamerstijl-stadspagina.
//
// Een nieuwe stad toevoegen = 3 dingen, geen gekopieerde SEO/schema:
//   1. city.landingIntro + city.landingOutro in src/data/cities.ts
//   2. foto in public/badkamerstijl/steden/<slug>.avif (valt anders terug)
//   3. een route van 3 regels:
//        export const generateMetadata = () => cityLandingMetadata('<slug>');
//        export default function Page() { return <CityLandingRoute slug="<slug>" />; }
//      plus de slug uitsluiten in src/app/badkamerstijl/[city]/page.tsx.
//
// title, H1, meta-description en JSON-LD (LocalBusiness, Breadcrumb, FAQ)
// vullen zich volledig vanuit de stad-parameter, identiek voor elke stad.
// ───────────────────────────────────────────────────────────────────────
...
export function cityLandingMetadata(slug: string): Metadata {
...
export default function CityLandingRoute({ slug }: { slug: string }) {
  const city = getCityBySlug(slug);
  if (!city) notFound();
